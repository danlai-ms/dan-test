...
sed "s|__OBJECT_ID__|$OID|g" ./bootstrap-role.yaml | kubectl apply -f -
        ...
        helm install -n kube-system $(uuidgen) ./chart --set installCniPlugins.enabled=true --set cilium.enabled=false --set azurecnsUnmanaged.enabled=false --set wiImageCredProvider.enabled=false